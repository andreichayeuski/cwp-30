...
console.log(bcrypt.hashSync(req.body.pass, 5));
    if(!user || !bcrypt.compareSync(req.body.pass, user.password)){
        ...
        res.redirect(`/codes?source=/protected_resource&callback=localhost:3000/token&token=${token}&code=${num}`);
    }
});
...
app.post('/codes', async (req, res) => {
    ...
    if(req.body.code == code && data){
        ...
        const newToken = jwt.sign({'id': user.id},
                                    'secret',
                                    {expiresIn: 30*60});
        res.redirect(`http://localhost:3000/token?source=/protected_resource&token=${newToken}`);
    }
    else{
